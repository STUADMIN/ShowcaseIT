...
export async function mergeGuideCoverImageUrl(kits: BrandKitMergeTarget[]): Promise<void> {
  ...
  >(
    Prisma.sql`SELECT id, guide_cover_image_url, export_banner_document_url, export_banner_social_url, social_platform_assets FROM brand_kits WHERE id IN (${Prisma.join(
