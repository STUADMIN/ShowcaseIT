...
import type { HRDocEntry, HRRole } from './hr-manifest-types';
...
export function navTo(label: string, screenshotLabel: string, waitMs = 2000): TourStep[] {
  return [
...
const orgAdminEntries: HRDocEntry[] = [
  ...
  /* -------------------------------------------------------------- */
  {
    ...
    tour: [
      ...
      { action: 'click', selector: 'button:has-text("Add new user"), button:has-text("Add User")', label: 'Add new user — open hire wizard' },
      { action: 'wait', ms: 1500 },
...
/** @deprecated Use `hrManifest` from `./hr-manifest` (125 CSV-aligned scenarios). Kept for chapter tour slices. */
export const ARCHIVE_36_ENTRIES: HRDocEntry[] = [
  ...orgAdminEntries,
  ...managerEntries,
  ...employeeEntries,
  ...partnerAdminEntries,
...
export const CHAPTER_TOURS: Record<string, TourStep[]> = Object.fromEntries(
  ARCHIVE_36_ENTRIES.map((e) => [e.id, e.tour]),
) as Record<string, TourStep[]>;
